/* ze.c,  Ze Emacs, Public Domain, Hugh Barney, 2024, Derived from: Anthony's Editor January 93 */
#include <stdlib.h>
#include <assert.h>
#include <curses.h>
#include <stdio.h>
...
/* forward scan for end of line containing point pt */
point_t end_of_line_point(buffer_t *bp, register point_t pt)
{
    register char_t *p = ptr(bp,pt);
    while (bp->b_ebuf > p && *p != '\n')
        p = ptr(bp, ++pt);
    return pt;
}
...
/* move up 1 line */
point_t previous_line_point(buffer_t *bp, point_t off)
{
    return start_of_line_point(bp, start_of_line_point(bp, off) - 1);
}

/* move down one 1 line */
point_t next_line_point(buffer_t *bp, point_t off)
{
    point_t pt = end_of_line_point(bp, off);
    return  pt < pos(curbp, curbp->b_ebuf) ? ++pt : pt;
}

/* return the point of a column on the line containing pt */
...
void display()
{
    ...
    if (bp->b_point < bp->b_page)
        bp->b_page = start_of_line_point(bp,bp->b_point);
